...
task :edge do
  ...
  ENV['RAILS_BRANCH'] ||= 'trunk'
  
  checkout_path = File.join(ENV['RAILS_PATH'], ENV['RAILS_BRANCH'].gsub(/\//, '-'))
  ...
  unless File.exists?(checkout_path)
    ...
    get_framework_for checkout_path do |framework|
      system "svn co http://dev.rubyonrails.org/svn/rails/#{ENV['RAILS_BRANCH']}/#{framework}/lib #{checkout_path}/#{framework}/lib --quiet"
    end
  end
